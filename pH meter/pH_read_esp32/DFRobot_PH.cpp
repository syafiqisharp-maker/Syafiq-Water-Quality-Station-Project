/*!
 * @file DFRobot_PH.cpp
 * @brief Arduino library for Gravity: Analog pH Sensor / Meter Kit V2, SKU:
 * SEN0161-V2 (Adapted for ESP32)
 *
...
#define PHVALUEADDR                                                            \
  0x00 // the start address of the pH calibration parameters stored in the
       // EEPROM
#endif
...
#if defined(ESP32)
  this->_prefs.begin("ph_cal", false);
  this->_neutralVoltage = this->_prefs.getFloat("neutral", 1500.0);
  this->_acidVoltage = this->_prefs.getFloat("acid", 2032.44);
  this->_prefs.end();
#else
  EEPROM_read(PHVALUEADDR,
              this->_neutralVoltage); // load the neutral (pH = 7.0)voltage of
                                      // the pH board from the EEPROM
  if (EEPROM.read(PHVALUEADDR) == 0xFF &&
      EEPROM.read(PHVALUEADDR + 1) == 0xFF &&
      EEPROM.read(PHVALUEADDR + 2) == 0xFF &&
      EEPROM.read(PHVALUEADDR + 3) == 0xFF) {
    this->_neutralVoltage = 1500.0; // new EEPROM, write typical voltage
    EEPROM_write(PHVALUEADDR, this->_neutralVoltage);
  }
  EEPROM_read(PHVALUEADDR + 4,
              this->_acidVoltage); // load the acid (pH = 4.0) voltage of the pH
                                   // board from the EEPROM
  if (EEPROM.read(PHVALUEADDR + 4) == 0xFF &&
      EEPROM.read(PHVALUEADDR + 5) == 0xFF &&
      EEPROM.read(PHVALUEADDR + 6) == 0xFF &&
      EEPROM.read(PHVALUEADDR + 7) == 0xFF) {
...
float DFRobot_PH::readPH(float voltage, float temperature) {
  float slope =
      (7.0 - 4.0) /
      ((this->_neutralVoltage - 1500.0) / 3.0 -
       (this->_acidVoltage - 1500.0) /
           3.0); // two point: (_neutralVoltage,7.0),(_acidVoltage,4.0)
...
void DFRobot_PH::calibration(float voltage, float temperature, char *cmd) {
  ...
  sCmd.toUpperCase();
  phCalibration(
      cmdParse(sCmd.c_str())); // if received Serial CMD from the serial
                               // monitor, enter into the calibration mode
}
...
void DFRobot_PH::calibration(float voltage, float temperature) {
  ...
  if (cmdSerialDataAvailable() > 0) {
    phCalibration(cmdParse()); // if received Serial CMD from the serial
                               // monitor, enter into the calibration mode
  }
}
...
void DFRobot_PH::phCalibration(byte mode) {
  ...
  case 1:
    ...
    Serial.println(F(">>>Please put the probe into the 4.0 or 7.0 standard "
                     "buffer solution<<<"));
    Serial.println();
    break;
  ...
  case 2:
    if (enterCalibrationFlag) {
      if ((this->_voltage > 1322) &&
          (this->_voltage < 1678)) { // buffer solution:7.0{
        Serial.println();
      ...
      } else if ((this->_voltage > 1854) &&
                 (this->_voltage < 2210)) { // buffer solution:4.0
        Serial.println();
...
#if defined(ESP32)
          this->_prefs.begin("ph_cal", false);
          this->_prefs.putFloat("neutral", this->_neutralVoltage);
          this->_prefs.end();
#else
          EEPROM_write(PHVALUEADDR, this->_neutralVoltage);
#endif
...
#if defined(ESP32)
          this->_prefs.begin("ph_cal", false);
          this->_prefs.putFloat("acid", this->_acidVoltage);
          this->_prefs.end();
#else
          EEPROM_write(PHVALUEADDR + 4, this->_acidVoltage);
#endif
