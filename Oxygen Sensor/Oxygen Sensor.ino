...
void querySensorData() {
  ...
  if (satSuccess && conSuccess && tempSuccess) {
    ...
    // --- PLAUSIBILITY CHECKS ---
    // Reject NaN, Inf, or physically impossible values before printing.
    // Valid ranges: DO Sat 0-200%, DO Conc 0-25 mg/L, Temp -5 to 50 °C.
    bool valuesOk = true;
    if (isnan(doSaturation) || isinf(doSaturation) || doSaturation < 0.0 || doSaturation > 200.0) {
      Serial.println("[WARN] DO Saturation value out of plausible range (0-200%), discarding reading.");
      valuesOk = false;
    }
    if (isnan(doConcentration) || isinf(doConcentration) || doConcentration < 0.0 || doConcentration > 25.0) {
      Serial.println("[WARN] DO Concentration value out of plausible range (0-25 mg/L), discarding reading.");
      valuesOk = false;
    }
    if (isnan(temperature) || isinf(temperature) || temperature < -5.0 || temperature > 50.0) {
      Serial.println("[WARN] Temperature value out of plausible range (-5 to 50 degC), discarding reading.");
      valuesOk = false;
    }

    if (!valuesOk) {
      Serial.println("[WARN] One or more values failed plausibility check. Reading skipped.\n");
      return;
    }

    // All values are plausible — print them
